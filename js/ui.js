...
function updateSiteStatus(isAvailable) {
    ...
    if (isAvailable) {
        statusEl.innerHTML = '<span class="text-red-500">●</span> 可用';
    } else {
    ...
    historyList.innerHTML = history.map(item => {
        ...
        if (item.episodes && Array.isArray(item.episodes) && item.episodes.length > 0) {
            ...
            const syncStatus = item.lastSyncTime ?
                `<span class="text-red-400 text-xs" title="剧集列表已同步">✓</span>` :
                `<span class="text-red-400 text-xs" title="使用缓存数据">⚠</span>`;
        ...
        <div class="bg-[#191919] rounded-lg p-6 max-w-md w-full max-h-[90vh] overflow-y-auto relative">
            ...
            <div class="mb-0">
                ...
                <div class="flex justify-end space-x-2">
                    ...
                    <button id="cancelBoxModal" class="ml-2 bg-red-700 hover:bg-red-800 text-white px-4 py-1 rounded">取消</button>
                </div>
            </div>
        </div>`;
                ...
                <div id="dropZone" class="w-full py-9 bg-[#111] rounded-2xl border border-[#333] gap-3 grid border-dashed">
                    ...
                    <div class="flex items-center justify-center gap-2">
                        <label>
                            ...
                            <div class="flex w-28 h-9 px-2 flex-col bg-red-700 rounded-full shadow text-white text-xs font-semibold leading-4 items-center justify-center cursor-pointer focus:outline-none">选择文件</div>
                        </label>
                        <button onclick="importConfigFromUrl()" class="flex w-28 h-9 px-2 flex-col bg-red-600 rounded-full shadow text-white text-xs font-semibold leading-4 items-center justify-center cursor-pointer focus:outline-none">从URL导入</button>
                    </div>
                    </div>
                </div>
            </div>
